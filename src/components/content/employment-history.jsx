...
function EmploymentHistory({ employmentHistory }) {
    return (
        <div className="flex flex-col gap-10 primary-flex w-full">
            <AccordionRoot collapsible variant="subtle" defaultValue={["Millennium Radius"]}>
                {employmentHistory.map((employment, index) => (
                    <AccordionItem value={employment.companyName} key={index} id={employment.id} className="my-2 px-4 py-5 duration-[0.4s] hover:bg-[#2F0553] "  bg={{ base: "transparent", _hover: "#2F0553", _open: "#2F0553" }}>
